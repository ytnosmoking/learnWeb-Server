<template>
  <div>
    <div class="number-panel">
      ...
      <p v-show="!checkedNumber" class="number-show"/>
      <ul>
        <li v-for="index in 9" :key="index" @click="clickThisNumber($event)">{{ index }}</li>
...
export default {
  name: 'TestPanel',
  components: {}, // 这里需要注意下，我们是采用的全局注入我们的组件，所以在后面因为我们的组件后，会直接使用这个命名的标签
  data() {
    return {
      checkedNumber: ''
    }
